...
public class Monday {
    ...
    @Test
    public void froilandaCropDustsEachCropRow() {
        // Given
        boolean expectedFertilizationStatusOfCrops = true;

        // When
        froilanda.fly(farm.getCropDusters().get(0), farm.getFields().get(0));
        List<CropRow> listOfCropRows = farm.getFields().get(0).getRowsInField();

        // Then
        for (CropRow eachCropRow : listOfCropRows) {
            for (Object eachCrop : eachCropRow.getCropsInRow()) {
                Crop crop = (Crop)eachCrop;
                Assert.assertEquals(expectedFertilizationStatusOfCrops, crop.hasBeenFertilized());
            }
        }
    }
}
